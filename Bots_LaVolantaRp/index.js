...
const commands = [
	new SlashCommandBuilder().setName('server').setDescription('La Volanta RP status'),
    new SlashCommandBuilder().setName('player').setDescription('Jugando A La Volanta RP'),
]
...
smithbot.once('ready', () => {
    ...
    setInterval(() => {
        smithmta.query({
            ...
            smithbot.user.setActivity(`La Volanta RP : ${state.raw.numplayers}/${state.maxplayers}`);
        }).catch(err => {
            console.log(err);
        });
    }, 5000);
